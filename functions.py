...
def remove_asc_primary_keys(data):
    (data, n) = re.subn(r'PRIMARY KEY(?P<before>(.*?)\(\s*\w*?\s*)ASC(?P<after>.*?\))', "PRIMARY KEY\g<before>\g<after>", data, flags=re.M | re.S)
    ...
    (data, n) = re.subn(r'UNIQUE(?P<before>(.*?)\(\s*\w*?\s*)ASC(?P<after>.*?\))', "UNIQUE\g<before>\g<after>", data, flags=re.M | re.S)
    ...
    (data, n) = re.subn(r'(?P<before>CREATE\s+(UNIQUE\s+)?INDEX(.*?)\(\s*\w*?\s*)ASC(?P<after>.*?\))', "\g<before>\g<after>", data, flags=re.M | re.S)
    for i in range(0, 20):
        (data, n) = re.subn(r'(?P<before>CREATE\s+(UNIQUE\s+)?INDEX(.*?)\(.*?,\s*\w*?\s*)ASC(?P<after>.*?\))', "\g<before>\g<after>", data, flags=re.M | re.S)
        if n == 0:
            break
